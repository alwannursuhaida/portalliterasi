...
// ─────────────────────────────────────────────
// 1. KONFIGURASI — ISI SESUAI PROJECT FIREBASE ANDA
// ─────────────────────────────────────────────
const CONFIG = {
  FIREBASE: {
...
async function onKelasChange() {
  ...
  try {
    const rows = await getWhere("siswa", "kelas", kelas);
...
async function loadLeaderboard() {
  try {
    ...
    let kpI = { kelas: "-", count: 0 };
    Object.entries(kelasCount).forEach(([k, c]) => {
      if (isPa(k) && c > kpA.count) kpA = { kelas: k, count: c };
      if (isPi(k) && c > kpI.count) kpI = { kelas: k, count: c };
    });

    document.getElementById("lb-siswa-putra").textContent      = spA.nama;
    document.getElementById("lb-siswa-putra-desc").textContent = `Kelas ${spA.kelas} | ${spA.count} Ulasan`;
    document.getElementById("lb-siswa-putri").textContent      = spI.nama;
    document.getElementById("lb-siswa-putri-desc").textContent = `Kelas ${spI.kelas} | ${spI.count} Ulasan`;
    document.getElementById("lb-kelas-putra").textContent      = kpA.kelas;
  ...
  try {
    ...
    container.innerHTML = items.map(u => {
      ...
      <div class="journal-item" style="border-left:4px solid var(--green-main)">
        ...
        <div style="font-size:12px;color:var(--ink);line-height:1.5;margin-bottom:6px"><strong style="color:var(--green-deep)">Kandungan:</strong> ${u.ulasanKandungan}</div>
        <div style="font-size:12px;color:var(--ink);line-height:1.5;margin-bottom:6px"><strong style="color:#2563eb">Kekuatan:</strong> ${u.kekuatanBuku}</div>
        <div style="font-size:12px;color:var(--ink);line-height:1.5;margin-bottom:6px"><strong style="color:#dc2626">Kelemahan:</strong> ${u.kelemahanBuku}</div>
